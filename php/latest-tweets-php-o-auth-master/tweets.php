<?php
	/* 
	 * various modification made from https://github.com/andrewbiggart/latest-tweets-php-o-auth/
	...
	**/
	...
	function display_latest_tweets(
		...
		// Function parameters.
		$twitter_user_id,
		$cache_file          = './tweets.json',  // Change this to the path of your cache file. (Default : ./tweets.txt)
			...
			if($connection){
				...
				if (count($get_tweets)) {
					...
					foreach($get_tweets as $tweet) {
						...
						$twitterItem["tweet_desc"] = $tweet_desc;
						$twitterItem["retweet_count"] = $tweet->retweet_count;
						$twitterItem["favorite_count"] = $tweet->favorite_count;
						...
						$twitterItem["time"] = $display_time;

						// Render the tweet.

						array_push($twitterList, $twitterItem);
	...
	display_latest_tweets('ACF_France');
?>
